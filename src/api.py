...
import json
import re
import uuid  # Necessário para gerar reportId em send_logs_to_backend
...
# Importações do seu código original (resumo daily-py - dev.txt)
...
post_schema = {
    ...
    "properties": {
        ...
        },  # String com valores específicos
        "publishedAt": {
            "type": ["string", "null"],
            "format": "date-time",
        },  # String (ISO 8601) ou null
...
# --- LÓGICA DE LIMPEZA DE PAYLOAD (ATUALIZADA PARA ACEITAR TIPOS) ---
...
def clean_post_payload(
    ...
    """
    Remove campos do payload do post que não são esperados pelo DTO do Spring Boot,
    baseado no tipo de post (principal, social, ou trending suggestion).
    """
    expected_fields = (
        EXPECTED_SOCIAL_FIELDS
        if is_social
...
# --- FUNÇÕES DE VALIDAÇÃO E COMUNICAÇÃO COM O BACKEND ORIGINAIS ---
...
def send_post(post_data, headers):
    ...
    cleaned_post_data = clean_post_payload(
        post_data, is_social=False
    )  # is_social=False para post principal
...
def send_social_post(post_data, headers):
    ...
    cleaned_post_data = clean_post_payload(
        post_data, is_social=True
    )  # is_social=True para post social

    try:
        validate_social_post(cleaned_post_data)  # Valida o payload já limpo
...
def send_trending_suggestions_to_backend(suggestions, headers):
    ...
    ]  # is_trending=True para sugestões
...
def send_logs_to_backend(log_data, headers=None):
    ...
    try:
        ...
        if "timestamp" in log_data and isinstance(log_data["timestamp"], datetime):
            ...
            # Se não fornecido ou não for datetime, usa o tempo atual em UTC
            timestamp = (
        ...
        payload = {
            ...
            "timestamp": timestamp,  # Usa o timestamp formatado
            "details": {
...
# Este esquema é usado para instruir o Gemini a retornar o JSON esperado para o conteúdo.
...
async def generate_content_with_gemini_service(
    ...
    Esta função é usada para a geração manual de conteúdo via endpoint FastAPI.
    """
