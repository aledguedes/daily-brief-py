...
try:
    ...
    JWT_SECRET = b"fallback_secret_para_evitar_erro_startup_insecure"
...
async def trigger_by_id(
    ...
    try:
        ...
        if not request_entry:
            logger.warning(f"Registro com ID {id} não encontrado no banco de dados.")
        ...
        logger.info(f"Parâmetros do BD para ID {id}: output_format='{output_format}', theme='{theme}'")

       
       
        output_report = await run_automation(output_format=output_format, theme=theme)
